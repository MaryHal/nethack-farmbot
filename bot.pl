...
my $autouser=""; # your login. leave blank to not log in automatically
my $autopass=""; # your password
my $server="127.0.0.1";    # the server name or ip address
my $port="23";     # the server port to connect to

my $god = "lawful";   # your god. may be name or alignment
...
sub pickup {#{{{
    ...
    while (1){
        ...
        if ($nhmessage =~ /You have.*extreme/){
            $nhmessage=0;
            debug "pickup: extreme weight problems";
            sendnow "q";
            return $pickedup;
        }
    }
